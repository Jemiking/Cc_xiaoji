...
class RecurringTransactionViewModel @Inject constructor(
    ...
    val recurringTransactions: StateFlow<List<RecurringTransactionItem>> = 
        ledgerApi.getAllRecurringTransactions()
            .stateIn(
    ...
    fun showEditDialog(transaction: RecurringTransactionItem) {
        _uiState.update { 
            it.copy(
                showDialog = true, 
    ...
    fun saveRecurringTransaction() {
        viewModelScope.launch {
            ...
            if (state.editingTransaction != null) {
                ledgerApi.updateRecurringTransaction(
                ...
                ledgerApi.createRecurringTransaction(
                    name = state.name,
                    accountId = state.accountId,
                    amountCents = state.amountCents,
                    categoryId = state.categoryId,
    ...
    fun toggleEnabled(transaction: RecurringTransactionItem) {
        viewModelScope.launch {
            ledgerApi.toggleRecurringTransactionEnabled(transaction.id)
        }
    }
    
    fun deleteRecurringTransaction(transaction: RecurringTransactionItem) {
        viewModelScope.launch {
            ledgerApi.deleteRecurringTransaction(transaction.id)
        }
    }
    ...
    fun getFrequencyText(transaction: RecurringTransactionItem): String {
        return when (transaction.frequency) {
            ...
            "MONTHLY" -> "每月${transaction.dayOfMonth}日"
            "YEARLY" -> "每年${transaction.monthOfYear}月${transaction.dayOfMonth}日"
...
data class RecurringTransactionUiState(
    ...
    val editingTransaction: RecurringTransactionItem? = null,
    val name: String = "",
