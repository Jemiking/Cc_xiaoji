...
if "%choice%"=="1" (
    ...
    if exist "app\build\outputs\apk\debug\app-debug.apk" (
        ...
        echo    · Android Studio: Build Variants 选择 debug 后 Ctrl+F9
        echo    · 命令行: scripts\windows\build_debug.bat
    )
) else if "%choice%"=="2" (
    echo.
    echo 正在安装Release版本...
    ...
    if exist "app\build\outputs\apk\release\app-release.apk" (
        ...
        echo    · Android Studio: Build Variants 选择 release 后 Ctrl+F9  
        echo    · 命令行: scripts\windows\build_release.bat
    )
) else if "%choice%"=="3" (
    echo.
    echo 正在卸载Debug版本...
