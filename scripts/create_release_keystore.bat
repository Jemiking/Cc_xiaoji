...
echo.
echo 生成密钥库...
echo 位置: D:\kotlin\Cc_xiaoji\ccxiaoji_release.keystore

keytool -genkey -v -keystore ccxiaoji_release.keystore -alias %alias_name% -keyalg RSA -keysize 2048 -validity %validity%0 -keypass %key_password% -storepass %keystore_password%
...
if %errorlevel% equ 0 (
    ...
    echo 密钥库创建成功！
    echo ================================
    echo 文件位置: ccxiaoji_release.keystore
    ...
    echo.
    echo 下一步：运行 configure_signing.bat 配置构建脚本
    echo ================================
) else (
    echo.
