...
class DatabaseBackupManager @Inject constructor(
    ...
    suspend fun createBackup(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            // 3. 获取当前数据库文件
            val currentDbFile = context.getDatabasePath(CcDatabase.DATABASE_NAME)
            ...
            val walFile = File(currentDbFile.parent, "${CcDatabase.DATABASE_NAME}-wal")
            val shmFile = File(currentDbFile.parent, "${CcDatabase.DATABASE_NAME}-shm")
    ...
    suspend fun restoreBackup(backupPath: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            // 1. 获取目标数据库文件
            val targetDbFile = context.getDatabasePath(CcDatabase.DATABASE_NAME)
            
            // 2. 创建当前数据库的临时备份（以防恢复失败）
            val tempBackup = File(targetDbFile.parent, "${CcDatabase.DATABASE_NAME}.temp")
            ...
            try {
                ...
                if (walBackup.exists()) {
                    walBackup.copyTo(File(targetDbFile.parent, "${CcDatabase.DATABASE_NAME}-wal"), overwrite = true)
                }
                if (shmBackup.exists()) {
                    shmBackup.copyTo(File(targetDbFile.parent, "${CcDatabase.DATABASE_NAME}-shm"), overwrite = true)
                }
